...
import { heroContent } from '../data/contentData';
import { gradientPalettes } from '../data/gradientPalettes'; // ✅ 추가
...
// ✅ CSS용 linear-gradient 문자열 생성
const createGradient = () => {
  const colors = getGradientColors();
  const colorStops = colors
    .map((color, index) => {
      const percentage =
        colors.length > 1 ? (100 / (colors.length - 1)) * index : 0;
    ...
    >
      ...
      <Canvas
        style={{
          ...
          height: '100%',
          zIndex: 0,
          pointerEvents: 'none', // 버블 위에서도 클릭 가능하게
        }}
        shadows
        dpr={[1, 1.5]}
        gl={{
          antialias: true,
          powerPreference: 'high-performance',
          alpha: true, // ✅ 배경 투명하게 (그라데이션이 뒤에서 보이도록)
          stencil: false,
        }}
        camera={{ fov: 60, position: [0, 0, 20], near: 0.1, far: 100 }}
      >
